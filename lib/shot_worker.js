...
const ShotWorker = function ( job, wsEndpoint ) {
	this.job = job
	this.wsEndpoint = wsEndpoint

	this.name = this.job.name;
	this.url = this.job.url;
	this.isBase = this.job.isBase;
	this.w_size = parseInt( this.job.width )
	this.h_size = parseInt( ( this.w_size * (9/16) ) )
	if ( this.w_size <= 720 ) {
		this.h_size = parseInt( ( this.w_size * (16/9) ) )
	}

	this.wId = ( sh.unique( this.job.url + '_' + this.job.width ) + '_' + this.job.width ).blue;
	this.slowMo = global.config.slowMo;

	console.log( ( 'New Worker ' + this.wId + ' spawned' ).green.bold );
}

ShotWorker.prototype.run = function () {
	let self = this;
		...
		(async () => {
			...
			await browser.newPage().then(async page => {
				await page.goto( self.url, {waitUntil: 'networkidle0', timeout: 120000} ).then( async function () {
						...
						if ( ( clipY + clipH ) <= height ) {
							...
							await new Promise(resolve => {
								part.on('parsed', async function() {
									await this.bitblt(snapFull, 0, 0, w_size, clipH, 0, clipY)
									if ( global.config.verbose ) {
										let buffer = PNG.sync.write(part);
										let path = 'debug/' + global.config.name + '_part_' + w_size + '_' + count + '_.png';
					...
					let dir = 'latest/' + global.config.name;
					let path = global.localConfig.galleryPath + '/history/' + global.config.name + '/' + self.name + '_' + w_size + '.png';
					if ( self.isBase ) {
						dir = 'history/' + global.config.name;
					} else if ( global.config.forceUpdate || ! new pathManager().fileExists( path ) ) {
						let historySnap = snapFull;
						let buffer = PNG.sync.write(historySnap);
						fs.writeFileSync( path, buffer);
						if ( global.config.verbose ) {
							console.log('Worker ' + self.wId + ': Saving "' + path + '"');
						}
					}
					let buffer = PNG.sync.write(snapFull);
					path = global.localConfig.galleryPath + '/' + dir + '/' + self.name + '_' + w_size + '.png';
					fs.writeFileSync( path, buffer);
					if ( global.config.verbose ) {
						console.log('Worker ' + self.wId + ': Saving "' + path + '"');
					}
				...
				await page.close();
				console.log( ( 'Worker ' + self.wId + ' finished' ).bold.green );
				resolve();
			});
		})()
