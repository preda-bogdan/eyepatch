...
Job.prototype.run = function ( finishedCallback ) {
	...
	new pathManager( { name: 'history' } ).ensureExists( function (err) {
		if (err) console.error( 'Unable to create history resource', err )
	} )

	new pathManager( { name: 'latest' } ).ensureExists( function (err) {
		if (err) console.error( 'Unable to create latest resource', err )
	} )

	//process.exit( 0 )

	puppeteer.launch({ headless: true, slowMo: global.config.slowMo, args: ['--no-sandbox', '--disable-setuid-sandbox', '--remote-debugging-port=9222' ]}).then(async browser => {
		const promises=[];
		for( let index in self.sizes ) {
			const worker = require('./worker.js').worker;
			promises.push( new worker( { url: self.url, size: self.sizes[index], wsEndpoint: browser.wsEndpoint(), isBase: false } ).run().catch( function(err) {
			...
			if( self.url_base ) {
				promises.push( new worker( { url: self.url_base, size: self.sizes[index], wsEndpoint: browser.wsEndpoint(), isBase: true } ).run().catch( function(err) {
					console.error( ( err ).red );
				} ) );
			}
		}
