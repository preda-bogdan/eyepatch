		...
		(async () => {
			...
			await browser.newPage().then(async page => {
				await page.goto( self.url, {waitUntil: 'networkidle0', timeout: 120000} ).then( async function () {
					...
					let dir = 'latest';
					let path = global.localConfig.galleryPath + '/history/' + global.config.name + '_' + self.name + '_' + w_size + '.png';
					if ( self.isBase ) {
						dir = 'history';
					...
					path = global.localConfig.galleryPath + '/' + dir + '/' + global.config.name + '_' + self.name + '_' + w_size + '.png';
					fs.writeFileSync( path, buffer);
					if ( global.config.verbose ) {
						console.log('Worker ' + self.wId + ': Saving "' + path + '"');
					}
